...
cut: quality of the cut color: diamond color clarity: measurement of how
clear the diamond is

### **q2** Run `glimpse(diamonds)`; what variables does `diamonds` have?

price, carat, cut, color, clarity, x, y,z dpeth, table
...
Carat: Vary from .2 - 2 with a extreme at 5, around .7 Cut is mostly
ideal color: G is most Clarity: SI1
...
-   (Write your observations here!) I don't understand how X,Y,Z. The
    caret varies with extremes alhtough has an average of .8. Cut is
    mosltly postive. The collection of diamonds is good quaility which
    is also relfected in the cost.
...
```{r q5-task}
read_csv(./challenges/data/tiny.csv)
df_q5
```
