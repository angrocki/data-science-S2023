...
-   Within the script editor or a chunk
    ...
    -   `Alt` + `-` insert assignment operator (Option+-)

    -   `Shift` + `Cmd/Ctrl` + `M` insert pipe operator (cmd+shift+M)

    -   `Cmd/Ctrl` + `Enter` Run current line/section (cmd+return)

    -   `F1` (Note: on a Mac you need to press `fn` + `F1`) show help
        function at curser (fn`+`F1)

    -   `Cmd/Ctrl` + `Shift` + `C` comment or uncomment current
        line/section (cmd+shift+C)

-   Within R Markdown

    -   `Cmd/Ctrl` + `Alt` + `I` insert chunk(sweave and knitr) (cmd +
        Option + I)

-   Within a chunk

    -   `Shift` + `Cmd/Ctrl` + `Enter` Source the current document with
        echo (Cmd + shift + Return)

    -   `Ctrl` + `I` (`Cmd` + `I`) Reindent Lines (Cmd + I)
...
Click the red square at the topo right conor of the chunk

### **q4** How do you show the "Document Outline" in RStudio?

*Hint*: Try googling "rstudio document outline"

command + Shift + Oor side button next to table
