...
\- Since `position = "stacked"` is the default for `geom_col()`, we see
not the real `hwy` values, but effectively a sum at each `cty` value!
...
A 2-seater is the least fequent and a suv is the most common.
...
In this dataset: - `SUV`\'s are most numerous, followed
by `compact` and `midsize` - There are very few `2seater` vehicles
- `pickup`\'s and `SUV`\'s tend to have `4` wheel drive - `compact`\'s
and `midsize` tend to have `f` drive - All the `2seater` vehicles
are `r` drive
...
-   What was the difference between `v1` and `v2`?
    -   V2 had more data since it showed two different variables with
        different aesthetics while V1 only had 1 different type with two
...
Certain manufacturers seem to favor particular classes of car. For
instance, *in this dataset*: - Jeep, Land Rover, Lincoln, and Mercury
only have `suv`\'s - Audi, Toyota, and Volkswagen favor `compact` -
Dodge favors `pickup`
